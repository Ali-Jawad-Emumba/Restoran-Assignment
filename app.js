...
document.addEventListener("DOMContentLoaded", () => {
    document.addEventListener('scroll', ()=>{
        ...
        navBar.className=`nav-bar ${(scrollY>100)?'nav-bar-scrolled':'nav-bar-unscrolled'}`
  ...
  const showHidePatternError=(errorElement, varToCheck, regex=regexAlphabetsPattern)=>{
    if (varToCheck){
  ...
  const showHideNavMobileMenu=()=>{
    ...
    mobileNavMenu.style.display=mobileNavMenu.style.display==='none'?'block':'none' 
   }